...
- [x] **oneLineSummary 錯位修復**：ch02/04/07 手修 78 條 + 全站自動修 23 條（0603）。**✅ 0604 補完**：ch03/05/06/08/09/10/16/26/31/32/43/44/45/46/47/48/49/50/59 等大量章節改版殘留的舊課表 oneLine/analogy 全依實際標題重寫
- [x] ~~補「☕ 用人話講」白話總結~~ **✅ 0604 全站 100% 完成（1163/1163 課、全 75 章）**。〔0603 定標準時還剩 69 章；0604 逐章三合一全部補完〕標準：當第一次碰電腦的國中生、讀每課實際內容寫、大量生活化比喻
- [x] oneLineSummary：全站 1163 課 100% 有（0 缺）
- [~] **圖文解說圖**：🖼️ **A 區操作教學圖全做完**（29 張 + 燈箱；0604 補 create react/vue/nuxt + VS Code 介面/繁中圖規格）。**🔴 概念示意圖還沒做、等林董生圖**——0604 已把規格擴成 **B + D（前端框架含 CRA）+ E（Python/爬蟲/Node）+ F（補 ch01-25）共 110+ 張**、見 `LESSON_IMAGE_SPEC.md`，生好放 `public/lesson-img/chNN/` 我依檔名插入

### E2 章節偷懶 / 未補（章節 audit）
- [x] **✅ 0604 ch33「Rust 完整」整章重寫**（原內容被 SEO 行銷汙染→還原成 25 課完整 Rust 課程 + 用人話講 + rust playground）
- [~] **🆕 0604 新增 ch76「Angular 完整」**（前端三大框架補齊，25+ 課，排在前端框架區 sortIndex 9.x）— 進行中
- [ ] **strict-spec 重寫**（跟用人話講不同層次的「嚴格規格重做」、可選）：ch68 高階工程師(20) 按 spec、ch55-58 / 60-67 / 69-70 strict rework。〔註：這些章「用人話講」0604 已補、內容已可用；strict rework 是更高標準的選配〕
...
## ✅ 本 session（0604）已完成（給 context、原項目上方已劃掉）

- **用人話講全站 100%**（1163/1163 課、全 75 章）+ 大量章節 oneLine/analogy 錯位修正 + ~100 個佔位 playground 換成各章主題範例
- **ch33「Rust 完整」整章重寫**（原 SEO 汙染→真 Rust 25 課）
- **程式碼區塊全站美化**（macOS 風 + 自動語法上色 + 終端機指令小抄）
- **Ch00 新手強化**（開工前 SOP + 每課「你會卡在哪」+ VS Code 繁中/介面圖；分流路徑記待辦）
- **AI 聊天拖拉上傳圖片**（拖圖進視窗直接上傳）
- **桌機章節大綱側欄不再蓋內容**（main 伸縮式留側欄寬）— 對應 I1 web_front RWD 一部分
- **首頁章/課數改吃 DB 即時資料**（不再用過時 build 快照）— 對應 I2 lesson_count
- **圖片規格擴充** A 區補 create 指令 + D/E/F 區（前端框架含 CRA / Python 爬蟲 Node / 補 ch01-25），共 110+ 張待生
- **🆕 ch76「Angular 完整」**（前端三大框架補齊，進行中）

> ⚠️ 程式碼變更（區塊美化、指令小抄、拖拉上傳、側欄、首頁數字）需 Zeabur **redeploy** 才生效；內容/用人話講已進 DB 即時生效。
