...
- [ ] 2.1 情境模板資料
  - [ ] 2.1.1 情境庫（談加薪/拒絕邀約/道歉/催款/請假/跟房東/跟老師/跟客戶/跟前任/客訴回應…）
    - [ ] 2.1.1.1 每情境：預設要點欄位 + prompt 模板 + 建議語氣
  - [ ] 2.1.2 語氣選項（客氣/專業/強硬/幽默/溫暖）
- [ ] 2.2 後端
  - [ ] 2.2.1 API `/api/message-coach`（POST {scenario,points,tone}→2–3 版本、`completeForUsage`）
  - [ ] 2.2.2 免費每日 3 則計數 + 付費無限
- [ ] 2.3 前端 `/message-coach`
  - [ ] 2.3.1 選情境→填要點→選語氣→產多版本（可複製、可微調「再委婉/再短一點」）
  - [ ] 2.3.2 RWD + 亮暗
- [ ] 2.4 變現：免費每日 3 則、付費/Z幣無限 + 進階語氣
- [ ] 2.5 收尾檢查
