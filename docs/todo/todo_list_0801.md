...
- [~] 5.7 🆕 **每日情報儀表板 `/daily`**（widget.md / 241.jpg · 每天回來看的入口）
  ...
  - [ ] Phase 2 **可自訂/拖拉 widget + Theme/Background/字體（照抄 Space·github.com/luffysky/snowrealmspace）**——已用 subagent 完整盤點 Space 架構，port 計畫如下（**建議另開專注 session 做，勿在馬拉松尾巴硬幹、會弄壞全站主題**）：
    - **2a 主題 palette（先做·但有雷）**：Space `packages/theme-engine`（types/presets/compile/color/contrast）13 色 token（primary/secondary/accent/background/surface/surfaceAlt/textPrimary/textSecondary/border/success/warning/danger/focusRing）+ ~21 招牌主題(粉霧/夜/紙/清晨/森/海/櫻/薰衣草/珊瑚/薄荷/墨/炭…)+~82 生成。**AI 島用 Tailwind `--color-*`、Space 用 `--sr-*`**→不整包搬引擎，改把 Space palette **映射到 AI 島 `--color-*`** 成 `[data-theme="名"]` 區塊(全站含聊天框自動換色)。⚠️**雷**：AI 島把 palette 與亮/暗擠在同一個 `data-theme`、`@custom-variant dark` 綁 `html:not([data-theme=light])`→加「亮色系 palette」會被當暗色。**必須拆軸**：`data-theme`=palette + 新 `data-mode`=light/dark，改 custom-variant + ThemeToggle 存兩軸。lessons 排除＝lesson 容器內把 `--color-*` reset 回預設。持久化：先 localStorage、之後 DB(`themes` 表 + `profiles.active_theme`)。
    - **2b Background + Lottie/程序化**：Space `components/BackgroundLayer.tsx`(image/video/gradient/procedural/lottie+濾鏡/裁切/glass)、`lib/api/background-resolver.ts`、`ProceduralScene`+`lib/scenes`、`lottie-web`。DB `background_items`/`background_playlists`/`playlist_items`。AI 島已有 `LottieBackground`——擴成可選背景庫 + 後台管理 + 「下載安裝」式套用。
    - **2c 字體 + 後台字體安裝器**：Space `fonts`/`font_pairs` 表 + `lib/theme/server-fonts.ts`/`font-loader.ts`；Space 後台有字體安裝器 → **AI 島 admin 也複製一個**（上傳/選 Google 字體→存表→`--font-*` 注入、主題可選字體）。
    - **2d 拖拉 widget 引擎**：Space `packages/widget-engine`(registry/grid/config-fields·純 zod 免 dnd 套件)、`components/widgets/{WidgetGrid,registry,WidgetBoundary}`、`app/(space)/home/{page,HomeGrid,WidgetSettings}`、API `widget-definitions`/`layouts`/`layouts/[id]/widgets(/bulk)`/`widgets/[id]`、DB `layouts`/`widget_instances`。**多租戶 `space_id`→改 `user_id`**、去掉 `x-space-id` header。手刻 Pointer-Events 拖拉+鍵盤(WCAG)，無外部 grid 套件。
...
- [ ] 4.2.1 續寫到 5000（現 **2348**，從 `dictionary-seed-65.json` 接）
  ...
  - [ ] 4.2.1.5 🆕 **前端基礎大補（HTML/CSS/JS 白話詞條）**——現有 seed 偏後端/系統/演算法，**前端「零基礎入門級」的具體元素/屬性/語法還很薄**（seed-44/45/61 只補了進階黑話：grid/BFC/reflow/shadow-DOM…，沒有 `<div>`、`display`、`let` 這種最基本的）。規劃 **8 批 ≈ 160 條**，每條照辭典體例（slug/term/zh_name/category/langs/plain 白話/analogy 比喻/example 小範例/related/difficulty，難度多為 1）。**author 前先跑 4.2.1.1 slug 去重掃描**（不少已收，如 flexbox/grid/promise/closure/spread-rest）：
    - [ ] 4.2.1.5.1 **HTML 元素·語意**（category=reference，~22）：`html-div`/`html-span`/`html-anchor`/`html-img`/`html-list`(ul/ol/li)/`html-table`/`html-heading`(h1-h6)/`html-paragraph`/`html-semantic-tags`(header/nav/main/section/article/aside/footer)/`html-media`(video/audio)/`html-canvas`/`html-iframe`/`html-head-tags`(meta/link/script/style)/`doctype`/`html-attribute`(id/class/data-*)/`html-entity`。
    - [ ] 4.2.1.5.2 **HTML 表單·輸入**（~18）：`html-form`/`input-types`(text/email/password/number/date/file/range/color)/`checkbox`/`radio-button`/`html-select`/`textarea`/`html-button`/`label-for`/`placeholder`/`required-attr`/`form-action-method`/`html-validation`/`fieldset-legend`/`autocomplete-attr`。
    - [ ] 4.2.1.5.3 **CSS 選擇器·特異性·盒模型**（~20）：`css-selector`/`class-selector`/`id-selector`/`attribute-selector`/`descendant-child-selector`/`pseudo-class`(:hover/:focus/:nth-child/:not)/`pseudo-element`(::before/::after)/`box-model`/`box-sizing`/`margin`/`padding`/`border`/`css-display`(block/inline/inline-block/none)/`css-cascade`/`inheritance-css`。（特異性 specificity 已在 61、去重）
    - [ ] 4.2.1.5.4 **CSS 排版·定位**（~18）：`flexbox-basics`(justify-content/align-items/flex-grow/flex-wrap)/`css-position`(static/relative/absolute/fixed/sticky)/`float-clear`/`css-overflow`/`vertical-centering`。（grid/BFC/z-index/stacking-context/container-query 已在 44、subgrid 已在 44 → 只補基礎與缺口）
    - [ ] 4.2.1.5.5 **CSS 單位·色彩·文字·外觀**（~20）：`css-units`(px/em/rem/%/vw-vh/ch)/`css-color`(hex/rgb/hsl/rgba/currentColor)/`background-css`/`border-radius`/`box-shadow`/`opacity`/`font-properties`/`line-height`/`text-align`/`letter-spacing`/`text-overflow-ellipsis`/`css-gradient`。（CSS 變數/巢狀/@keyframes/流體排版 已在 61、clamp 在 44 → 去重）
    - [ ] 4.2.1.5.6 **JS 基礎語法·型別**（~22）：`js-variable`(var/let/const)/`js-data-types`/`js-operator`/`ternary`/`truthy-falsy`/`equality-js`(== vs ===)/`hoisting`/`js-scope`/`arrow-function`/`default-params`/`destructuring`/`optional-chaining`/`nullish-coalescing`/`short-circuit`/`type-coercion-js`/`nan-undefined-null`。（closure/spread-rest/template-literal/tagged-template 已在 45 → 去重）
    - [ ] 4.2.1.5.7 **JS DOM·事件**（~20）：`dom`/`queryselector`/`getelementbyid`/`createelement`/`appendchild`/`addeventlistener`/`event-object`/`event-bubbling-capturing`/`event-delegation`/`preventdefault`/`classlist`/`innerhtml-vs-textcontent`/`dataset-js`/`domcontentloaded`/`form-submit-event`。（shadow-DOM/web-component 已在 45 → 去重）
    - [ ] 4.2.1.5.8 **JS 陣列/物件方法·非同步入門**（~20）：`array-map-filter-reduce`/`foreach`/`array-find-some-every`/`array-includes`/`array-sort`/`object-keys-values-entries`/`json-parse-stringify`/`array-from`/`async-await`/`fetch-js`/`then-catch`/`settimeout-setinterval`/`promise-all`。（promise/generator/iterator/microtask-macrotask/weakmap/structuredClone 已在 45 → 去重）
