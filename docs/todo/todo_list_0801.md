...
- [~] 5.7 🆕 **每日情報儀表板 `/daily`**（widget.md / 241.jpg · 每天回來看的入口）
  ...
  - [~] Phase 2 **可自訂/拖拉 widget + Theme/Background/字體（照抄 Space）**——2a/2b/2c 全上線（0806），2d 計畫已備：
    ...
    - [ ] **2d 拖拉 widget 引擎**：計畫 doc 已備 `docs/widget_homepage_port_plan.md`（DB per-user schema / 照抄 grid.ts·config-fields.ts·WidgetGrid·WidgetBoundary / widget 分批 / 6 API / 5 Phase）。使用者選「先只寫計畫」→ 待點頭開工。
