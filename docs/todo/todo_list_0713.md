...
- ~~**首頁 Hero 重做**：統一品牌三色、拿掉彩虹漸層/emoji 改 Lucide、大字排版＋留白、模式卡一致化、平順 hover/active。（0714）~~
- ~~**主視覺換實景**：Hero 用 GPT 生的漂浮 AI 島實景（`hero-island-dark/light`），**日/夜隨主題自動切**。（0714）~~
- ~~**連續世界 2.5D 骨架**：`WorldZone`(日→夜背景)＋`ParallaxLayer`/`ParallaxScene`(可抽換多圖層)＋`Reveal`(滾動淡入)＋星空景深；Hero→StageMap 無縫同世界。全尊重 reduced-motion。（0714）~~
- ~~**StageMap 互動活地圖**：接 GPT 的發光路徑層當底圖＋**HTML 資料節點**(可點/狀態化)；章數改真實資料(80 章)。（0714）~~
- ~~**滑順動畫**：hover 浮起/圖示微轉/CTA 位移＋framer-motion 進場＋視差景深。（0714）~~
- ~~**深/淺色都要合理**：世界背景/文字改隨主題日/夜切、走 CSS token；Hero 日夜雙圖。（0714）~~
- ~~**霧面玻璃**：模式卡/關卡卡/nav＋抽屜改 `.surface-glass`/backdrop-blur。（0714）~~
- ⬜ **完整 5 層視差場景**：等 GPT 交**同尺寸對齊**的五層(`stage-layer-01~05`，sky 16:9 vs 現在層 3:2 要統一) → 用 `ParallaxScene` 疊成真 2.5D（Hero 或關卡地圖擇一當展示）。
- ⬜ **各區塊重做**：吉祥物介紹（三張去背角色卡，等 char 圖）、副本＋魔王整併一區、精選章節、生涯路徑 — 逐區美化、去燒字舊圖。
- ⬜ **WorldMap 接真實進度**：把使用者完成度餵進 `stateFor()` → 自動算 done/current/locked。
- ⬜ **風格外溢**：把玻璃/動效/間距系統逐頁鋪到章節/分身島/機會島/辭典（一區一區換、每次驗亮暗+不破版）。
- ⬜ **其餘主視覺**：`index-img.md` 清單（夜景 CTA、模式小圖、char 去背）陸續生 → 接入。
