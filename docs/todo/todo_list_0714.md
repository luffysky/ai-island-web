...
- 🆕 **分身島多模式**：除了「分身任務(跑工具)」，加「💬 問問(純 LLM 聊天)」模式——只想問建議/腦力激盪時不跑整套 agent、直接 LLM 回（帶你的檔案+記憶+對話脈絡）。之後可再加：頭腦風暴/教練/評審/查資料 等模式切換。
...
- 🆕 **辦公室員工詳情 + 上/下線狀態**：點「N 位在職」→ 展開看**是哪些員工**、各自**在職/放養中/下線**狀態、最近在做什麼。
- 🆕 **員工「放養」模式（巡邏→匯報）**：讓員工「出去逛」——自主用 `web.search`/`web.research` 逛論壇/網站，把看到有趣/值得注意的**帶回來匯報**（可用語意或指令問「你剛看到什麼」）。上線=回公司、放養=在外巡邏。**做法＝派一個瀏覽型 agent 任務給該員工**（讀取型、安全）。
  - 🔴 **紅線**：「跑去別的論壇留言/聊天/發文」＝**對外動作**，一律**AI 起草→你批准才發**，且需該站帳號＋守該站 ToS，不可自主亂留言。所以 MVP 只做「**逛+讀+匯報**」（安全）；要留言＝走批准流程、之後接。
