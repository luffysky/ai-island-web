...
- [x] ~~定價頁改全免費・不販售 + 常見問題全改~~
- [x] ~~部落格 AI 輔助 SEO（/api/blog/ai-seo）+ GEO 區塊（強化 JSON-LD + AI 摘要 + /llms.txt）~~
- [x] ~~碎片搜尋（森林 + 引擎素材欄）~~
- [x] ~~演化碎片多選多對多 + 適合做什麼全選 + 綠寶聚焦選中碎片~~
- [x] ~~成長分工作室（範圍切換：個人島/各工作室/全部），修跨工作室加總 bug~~
- [x] ~~查看成員沒反應修好（展開/收合+載入中+中文角色）+ 操作記錄頁 /creator-island/activity~~
- [x] ~~引擎碎片素材跟碎片庫同步（載入全部）+ 編織 prompt 強化融合（不再東一塊西一塊）~~
- [x] ~~全站 TipTap 保真：sanitizer 補 font-size/對齊，發佈公開頁+草稿都不掉格式~~
- [x] ~~Header menu 重設計 + 加創作者島嶼~~
- [x] ~~破版收尾：排行榜規則卡/導覽氣泡/筆記 pill/程式碼工具列 + 全站溢出護欄確認~~
- 查證：碎片庫 `ci_fragment_pool` 實有 **10,009 句**；建工作室自動抽 **300** 給。
