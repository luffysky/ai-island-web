...
> 🔗 專項另冊：`todo_list_0916.md` = Nami 0916 反饋的大型內容工程「**ch26/ch27 程式碼註解改寫成初學者一行一行白話**」（ch27 L3 已立標竿、其餘 62 課待做）。
...
> ＊0823 進度摘要（16 commit）：**辭典 2474→2583**（+109，seed 75-81：React Hooks/TS 型別·雲端 AWS-GCP·觀測性·資料工程·資安進階·前端狀態·行動開發）；**機會島雷達 §3.3.1**（api/sitemap 來源，新 `lib/opportunity-radar.ts`+17 測試）**§3.3.2**（三層 hash 變動偵測 ETag→body→逐項）；**林董授權後 gated 項**：§7.0.1 SEO 轉址三件到齊（後台 CRUD+middleware 301/302+RLS migration）、§7.6 作業自動批改**核實早已做**（AI 建議→人確認，todo stale 修正）；**教具 §4.1.5 一次 18 章 100% 互動覆蓋**（ch13 SEO·ch59 一人公司·ch52 AI設計·ch55 AI行銷·ch43 專案管理·ch44 PdM·ch45 跨職能協作·ch60 創業心法·ch51/53/54/56/57/58 AI創作職涯·ch77 機器學習·ch78 深度學習·ch79 語言模型·ch68 高階工程師修煉·ch71 Debug 思維，約 +290 demo）→ **主線 1-60 + ML 77-79 + 68/71 全覆蓋**；純速查附錄 ch61-67/69-70 刻意不塞（避免「充數」）。全部 tsc/vitest(242)/build 綠、migration/辭典/章節皆 import DB。**未動（待林董決策）**：§6.8 付費 gating/Coco v1/付費章節（定價經濟決策）、§2.3.2 social adapter（需 bot token）。詳見 `daily_works_0823.md`。
...
- [~] 5.7 🆕 **每日情報儀表板 `/daily`**（widget.md / 241.jpg · 每天回來看的入口）
  ...
  - [~] Phase 2 **可自訂/拖拉 widget + Theme/Background/字體（照抄 Space）**——2a/2b/2c 全上線（0806），2d 計畫已備：
    - [x] ~~**2a 主題 palette + Theme Studio**~~ ✅ 0806：拆軸（`data-theme`→`data-mode` 亮暗 + `data-palette` 色盤）解掉雷；**再進化成完整 Theme Studio**——移植 Space `theme-engine` 進 `src/lib/theme/engine`（`--sr-*`→`--color-*` 重映）、`themes` 表 + `profiles.active_theme_id`、`/theme-studio` 編輯器（13 色/字型/表面/效果、live 預覽/preset/a11y）、API apply、SSR inline 注入(FOUC-free、重整保留)。header 具名色盤下拉 + me 側欄/頭像下拉入口。（`7dd7b571`/`64c5b6a0`/`6c158dfe`/`ddd70bc1`/`f81b3952`/`844d4d46`/`d9f0e4b6`）
    - [~] **2b Background**：[x] ~~Phase 4a——procedural 335 個 canvas 粒子場景 + gradient + 獨立 `/background` 頁 + BackgroundLayer(z-index:-10) + `profiles.active_background`/`ai_bg` + `backgrounds` bucket~~ ✅ 0806（`76301594`/`4d5b27f2`）。⬜ **Phase 4b 待做**：lottie 動態場景（dotlottie-wc）+ 自訂圖片上傳（bucket 已備）+ 後台管理。
    - [x] ~~**2c 字體 + 後台字體安裝器**~~ ✅ 0806：`fonts`/`font_pairs` 表 + `fonts` bucket、GET `/api/fonts` + font-loader（@font-face/`<link>`→`--font-*`）、`/admin/fonts` 上傳安裝器、**內建 Space 全字體目錄 25 支（19 支 Google Fonts 免上傳即裝 via css_url、6 支佔位待上傳）**、中文名 display_name（後台+選字下拉顯示）、Theme Studio 選字面板。（`2438d5d2`/`0f285b5b`/`fa11d977`/`397b24b6`）
    - [~] **2d 拖拉 widget 引擎**（0807 開工·MVP 跑通）：[x] ~~P1 引擎地基（migration widget_layouts/instances+RLS、grid.ts/config-fields.ts 照抄、registry 重寫 8 widget、types/presets）~~ [x] ~~P2 8 個 Phase A widget 實作+渲染器（WidgetBoundary/Suspense）~~ [x] ~~P3 /home 頁面+HomeGrid 拖拉/縮放/加入/移除/存 DB+3 API（add/bulk/patch-delete）+me 側欄入口~~；⬜ 待補：WidgetSettings 齒輪設定(zod 自動表單)、多 layout 版面管理、P4 補更多 Phase A widget、P5 Phase B data widget。計畫 doc `docs/widget_homepage_port_plan.md`。
...
> ＊**架構（島內新區·0804 定）**：不開獨立分站，做站內 `/pet` 新區——① nav 加「🐾 寵物」(四語 i18n) ② `/pet` 落地頁(四功能卡，比照 `FreeToolsSection`) ③ 寵物檔案表 `pets`(user_id/name/species/breed/birth_date/photo_url/…、RLS) 供各功能共用 ④ 各功能複用現成 infra(LLM `completeForUsage`、gating `hasAiUnlimited`/每日計數、cron+LINE 推播、運勢 `generateFreeFortune`、生圖) ⑤ 首頁「免費 AI 小工具」加寵物入口。
> ＊建置順序：6.1(漏斗神器·先) → 6.2(複用運勢·最省) → 6.3(訂閱黏著) → 6.4(候選)。每個都要真接後端、不做假功能。
> ⚠️ **命名衝突（0804 查到，動工前必解）**：站內**已有 `pets` 表 + `/me/pet` + `/api/pet/*`（chat/tick/evolve/quest）＝虛擬吉祥物寵物系統**（會進化/聊天/做任務）。§6 是「使用者的真實寵物」工具，**不可佔用 `pets` 表與 `/pet` 路由**。→ 定案：真寵物用**新命名空間** `/petcare`（或 `/pets` 複數）+ 新表 **`user_pets`**（user_id/name/species/breed/birth_date/photo_url…RLS），與吉祥物系統完全隔離。nav 標「🐾 寵物照顧」以區別吉祥物。
...
> ＊**repo 已開**（0804 林董）：`https://github.com/luffysky/ai-island-bridge`（**獨立 repo、不在本 workspace**）。升級桌面客戶端要 clone 該 repo 另開工作；本 repo 這側只做伺服器介面（device 表/命令派發/緊急停止）。屬跨 repo 專案、另立一輪處理，不塞進語音/寵物這批。
...
- [ ] 2.6.6 工作空間 Hub（簡報/表格/文件生成器聚合入口）🗂️ **有規劃書**：`docs/island/分身島規劃待閱.md` §二（務實切法：先做產出中心聚合、**明確不自建 slides/sheets 編輯器**、簡報走 MD→deck）
...
- [ ] 2.7.6 Cost-per-Task benchmark 實測；2.7.7 AI COO 🗂️ **有規劃書**：`docs/island/分身島規劃待閱.md` §三（站在 2.6.1+2.1.4 上的組織層自主規劃器·提案制不自動跑·排 2.6.4 之後）
- [ ] 2.7.8 MCP 外部實戰測試；2.7.9 MCP Marketplace + 技能市集分享 🗂️ **有規劃書**：`docs/island/分身島規劃待閱.md` §四（技能分享安全先做、外部工具來源 URL 只走站方 curated 審核）
...
- [ ] 4.1.5 🔴🔴 **互動學習道具大工程（林董 0723 定調·全站加密）**——「教具＝能讓使用者真正學懂該章的互動道具、玩了就懂，不是塞選擇題充數」。**規則**：
  ...
  - **進度**：
    ...
    - [x] ~~**AI 創作/職涯小章 ch51/53/54/56/57/58 100% 互動覆蓋**~~ ✅ 0823——6 章共補 23 個 GAP 課、全 0 GAP：ch51 AI寫作(4：設定/寫作SOP/變現誠實版/CRISP prompt)、ch53 AI短影音(4：Hook腳本/不露臉SOP/量產batching/接案報價 quiz)、ch54 AI音樂(4：製作/版權誠實版/voice cloning/三階報價 quiz)、ch56 AI虛擬IP(5：角色一致性/Live2D vs VRM quiz/貼圖上架/純AI主播倫理/變現)、ch57 AI法律倫理(3：生成內容著作權/偏見與幻覺/倫理自律)、ch58 AI職涯(3：學習金字塔/副業→Indie quiz/自我品牌)。紅線=未授權克隆他人聲音肖像/仿冒品牌/假親密誘導斗內/造假人設/未查證專業宣稱。import 6 章已同步 DB。
...
- [ ] 4.2.1 續寫到 5000（現 **2583**，從 `dictionary-seed-82.json` 接）〔0820 前端零基礎大補 +88→2436、常見錯誤訊息 +11→2447、開發者工具 +12→2459、終端機/命令列基礎 +15→2474；0823 React Hooks 深入+TS 進階型別 seed 75 +16→2490、雲端服務 AWS/GCP seed 76 +18→2508；0823b seed 77 可觀測性/監控 +16→2524、seed 78 資料工程 +16→2540、seed 79 認證/資安進階 +17→2557、seed 80 前端狀態管理 +13→2570、seed 81 行動開發 +13→2583〕
  ...
  - [~] 4.2.1.5 🆕 **前端基礎大補（HTML/CSS/JS 白話詞條）** — ✅ 0820 8 批唯一新 slug 全 author 完（88 條，撞既有 18 條已在庫）→ import 2436、無衝突——現有 seed 偏後端/系統/演算法，**前端「零基礎入門級」的具體元素/屬性/語法還很薄**（seed-44/45/61 只補了進階黑話：grid/BFC/reflow/shadow-DOM…，沒有 `<div>`、`display`、`let` 這種最基本的）。規劃 **8 批 ≈ 160 條**，每條照辭典體例（slug/term/zh_name/category/langs/plain 白話/analogy 比喻/example 小範例/related/difficulty，難度多為 1）。**author 前先跑 4.2.1.1 slug 去重掃描**（不少已收，如 flexbox/grid/promise/closure/spread-rest）：
    - [x] 4.2.1.5.1 ✅0820 **HTML 元素·語意**（category=reference，~22）：`html-div`/`html-span`/`html-anchor`/`html-img`/`html-list`(ul/ol/li)/`html-table`/`html-heading`(h1-h6)/`html-paragraph`/`html-semantic-tags`(header/nav/main/section/article/aside/footer)/`html-media`(video/audio)/`html-canvas`/`html-iframe`/`html-head-tags`(meta/link/script/style)/`doctype`/`html-attribute`(id/class/data-*)/`html-entity`。
    - [x] 4.2.1.5.2 ✅0820 **HTML 表單·輸入**（~18）：`html-form`/`input-types`(text/email/password/number/date/file/range/color)/`checkbox`/`radio-button`/`html-select`/`textarea`/`html-button`/`label-for`/`placeholder`/`required-attr`/`form-action-method`/`html-validation`/`fieldset-legend`/`autocomplete-attr`。
    - [x] 4.2.1.5.3 ✅0820 **CSS 選擇器·特異性·盒模型**（~20）：`css-selector`/`class-selector`/`id-selector`/`attribute-selector`/`descendant-child-selector`/`pseudo-class`(:hover/:focus/:nth-child/:not)/`pseudo-element`(::before/::after)/`box-model`/`box-sizing`/`margin`/`padding`/`border`/`css-display`(block/inline/inline-block/none)/`css-cascade`/`inheritance-css`。（特異性 specificity 已在 61、去重）
    - [x] 4.2.1.5.4 ✅0820 **CSS 排版·定位**（~18）：`flexbox-basics`(justify-content/align-items/flex-grow/flex-wrap)/`css-position`(static/relative/absolute/fixed/sticky)/`float-clear`/`css-overflow`/`vertical-centering`。（grid/BFC/z-index/stacking-context/container-query 已在 44、subgrid 已在 44 → 只補基礎與缺口）
    - [x] 4.2.1.5.5 ✅0820 **CSS 單位·色彩·文字·外觀**（~20）：`css-units`(px/em/rem/%/vw-vh/ch)/`css-color`(hex/rgb/hsl/rgba/currentColor)/`background-css`/`border-radius`/`box-shadow`/`opacity`/`font-properties`/`line-height`/`text-align`/`letter-spacing`/`text-overflow-ellipsis`/`css-gradient`。（CSS 變數/巢狀/@keyframes/流體排版 已在 61、clamp 在 44 → 去重）
    - [x] 4.2.1.5.6 ✅0820 **JS 基礎語法·型別**（~22）：`js-variable`(var/let/const)/`js-data-types`/`js-operator`/`ternary`/`truthy-falsy`/`equality-js`(== vs ===)/`hoisting`/`js-scope`/`arrow-function`/`default-params`/`destructuring`/`optional-chaining`/`nullish-coalescing`/`short-circuit`/`type-coercion-js`/`nan-undefined-null`。（closure/spread-rest/template-literal/tagged-template 已在 45 → 去重）
    - [x] 4.2.1.5.7 ✅0820 **JS DOM·事件**（~20）：`dom`/`queryselector`/`getelementbyid`/`createelement`/`appendchild`/`addeventlistener`/`event-object`/`event-bubbling-capturing`/`event-delegation`/`preventdefault`/`classlist`/`innerhtml-vs-textcontent`/`dataset-js`/`domcontentloaded`/`form-submit-event`。（shadow-DOM/web-component 已在 45 → 去重）
    - [x] 4.2.1.5.8 ✅0820 **JS 陣列/物件方法·非同步入門**（~20）：`array-map-filter-reduce`/`foreach`/`array-find-some-every`/`array-includes`/`array-sort`/`object-keys-values-entries`/`json-parse-stringify`/`array-from`/`async-await`/`fetch-js`/`then-catch`/`settimeout-setinterval`/`promise-all`。（promise/generator/iterator/microtask-macrotask/weakmap/structuredClone 已在 45 → 去重）
    - ＊完成一批就 import + 更新條數；全 8 批約 **2348 → ~2500**。之後可續「無障礙 a11y 入門級」「瀏覽器開發者工具用法」「常見前端錯誤訊息」等前端向主題。
...
- [~] 7.0.5 **DB schema 對照**：~~跑 `scripts/audit-db-columns.mjs`~~ ✅ 0820——無新增破綻；既有 ✗ 全是 `${POST_COLS}`/`${AUTHOR}` 這類 template-literal 欄位名 + route 前綴的靜態分析誤判，非真錯。逐表核對 migration 完整性（對照 upgrade.md「孤兒表」）待續。
