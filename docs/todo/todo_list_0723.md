...
- 🆕 **每日晨報（整合·CP 值最高·複用現成 infra）**：早上一則 LINE＝運勢 ＋ **AI 天氣生活建議**（穿搭/帶傘/曬衣/運動/健康提醒·綁節氣農民曆）＋ 今日待辦 ＋ 重要提醒。骨架已有（`/api/cron/daily-brief`「今天值得做的 3 件事」+ buildListCard Flex）。天氣資料源＝**中央氣象署開放資料 或 Open-Meteo（免 key、免費）**、零成本。＊林董 0723 問「天氣功能」→ 結論：純天氣別做（免費到處有）、做成晨報的一塊才黏。
- 🆕 **大眾變現候選**（林董 0723 腦力激盪）：統一發票對獎（最台·剛需·中獎情緒·兩月一期天然推播，財政部雲端發票 API）、AI 取名（寶寶/英文/店名/寵物+筆劃生肖·一次性付費意願高·複用命理 infra）、健康喝水提醒+streak、AI 解夢/心情陪伴、生日提醒+送禮（可接聯盟行銷）。
