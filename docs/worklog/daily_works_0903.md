...
## F. 修：「只要粒子」把飄雪/冰晶/花瓣壓成灰色小點（Nami 回饋）

**現象**：套「只要粒子」之後，本來的飄雪、飄冰、飄花在亮色主題下全變成一堆分不出誰是誰的細小灰點——「我說的只保留粒子是指那些會飄的東西」，不是把它們改成粒子。

**原因**：B 節那版的 `adaptColor()` 是**直接改粒子顏色**——淺底 + 亮粒子 → 把顏色壓到亮度 0.34。白雪(255,255,255) 就變成 (87,87,87) 的中灰、冰晶變灰星、櫻花變暗粉 → 形狀還在、但顏色沒了，看起來就是一律的小灰點。方向本身就錯：雪就該是白的。

**修法（`particle-color.ts` + `ProceduralScene.tsx`）**：
- `adaptColor()` → 換成 **`particleInk(color, bgLum)`**，回 `{ fill, halo }`。**`fill` 永遠是場景定義的原色**，一個 byte 都不改。
- 對比不夠時（亮度差 < `MIN_CONTRAST` 0.18）才給 `halo`：同色相、往對比方向推到底（淺底→亮度 0.1、深底→0.9）的顏色。
- 畫法＝**三層由外而內、越來越濃的同形狀描摹**（`HALO_RINGS` 半徑 +100%/62%/30%、alpha 0.10/0.13/0.17）疊出柔邊。實測比較過三種做法：單層濃暈會變成難看的「甜甜圈」、canvas `shadowBlur` 會把冰晶的星形跟花瓣糊成一團（而且慢），三層淡的最好、也最便宜。粒子 > 350 顆時少畫最外圈。
- 描邊型形狀（雨絲 `streak` / 氣泡 `ring`）的暈是**加粗**不是放大（放大會把雨絲拉長）。
- `drawShape()` 因此改成收 `(col, r, lw)`，本體與暈共用同一段繪製碼。

**實測（用站台實際底色 rgb(250,251,252) 逐一畫出來對照）**：細雪＝白雪花帶柔灰邊、冰晶＝看得出星形、櫻花（亮度差 0.19 > 門檻）＝**完全不動**還是原本的粉、細雨＝看得見的雨絲。門檻 0.18 就是照這組實測抓的。

**驗證（鐵規則）**：
- `tests/background-particles.test.ts` 改寫成 `particleInk` 的 14 個 case（原色不動 / 淺底加深暈 / 深底加亮暈 / 暈保留色相 / 對比夠不加暈 / 壞字串不炸 + `isParticlesOnly`）
- `npx tsc --noEmit` ✅、`npx vitest run` ✅ **35 檔 256 測試**、`npx next build` ✅
- API/DB：未動；UI：只動 canvas 繪製，沒有新元素 → RWD／桌面／PWA 不受影響
