...
AI 功能為本平台之核心，其 API 呼叫為主要變動成本。本平台已實作**多層成本控制**（詳第四章 4.3），確保此項成本**具月度硬性上限、不會失控**。

**（一）現行系統金鑰月預算與實際支出**（✅ 資料庫實查，[待補] 金額以現行設定為例）：
...
> 關鍵事實：任一金鑰達月預算即**自動停用該供應商並切換備援**，全部達上限時服務降級為「暫時忙碌」而**不再產生費用**（`chat/route.ts:135-151`）。因此本平台單月 AI 成本**具明確上限（現行約 US$90），可隨營收成長逐步調高**，不存在「流量爆量導致帳單暴增」之風險。
...
**（三）規模化成本推估**（推論，[待補] 待實測校準）：以免費用戶主要走低成本 / 免費模型估算，於現行每人上限下，數百名活躍免費用戶之月度 AI 成本仍可控制於數十美元級距；付費用戶則以訂閱 / 儲值覆蓋其用量。**AI 成本與營收具正相關而非失控關係。**
...
- **成本性質**：方案制固定月費，隨資料量與流量緩增；目前資料規模（80 章 / 1,258 節課 / 千級用戶資料）尚屬輕量。
...
- **內容翻譯**：採用免費翻譯端點（非計費 API），四語互譯**零邊際成本**；並以雜湊快取「只翻異動內容」，進一步降低呼叫量（`gtranslate.ts`、`content-i18n.ts`）。此為本平台相對競品之**顯著成本優勢**——多語內容通常為高成本項目，本平台將其降至趨近於零。
...
- **人力成本不隨用戶數成長**，屬準固定成本。
- `[待補:需老闆填]` AI 輔助開發工具（如 Claude Code 訂閱）月費，作為研發工具成本列示；此為「以工具成本換取人力成本」之策略，總體研發成本遠低於傳統團隊。

> 誠實界定：單人團隊為現況，亦為風險（詳 6.8 與第八章）；補助經費之用途之一即為擴充團隊、降低單點依賴。
...
1. **成本可預測、有硬性上限**：最大變動成本（LLM API）已設月度硬上限（現行約 US$90），失控風險已於工程層面消除。
2. **零成本差異化**：多語翻譯此一通常高成本之功能，本平台以自建管線降至零邊際成本。
3. **低人力結構**：AI 輔助開發使研發成本不隨規模成長，單位經濟具優勢。
4. **與營收正相關**：AI 成本隨付費用戶成長，而非與流量脫鉤失控，具健康之單位經濟基礎。
