...
## 🔜 剩下（之後分批）
- emoji 還有零星：home Hero/MissionDungeons/MascotIntro 的主題性 emoji（刻意保留行銷設計）、ResourceCard 成長刻度(🌱🌿🌳)、無對應 lucide 的 🐹🗺️👹🐍 等。
- engine-types 的 type `emoji:` 欄位（型別識別 glyph，未改；要改需把 CreationType 改帶 Icon component）。
