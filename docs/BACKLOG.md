...
| 命令 | 功能 | 工時 |
|---|---|---|
| `/today` | 立即查今日學習狀況（不用等 20:00）| 20 min |
| `/weak` | 列自己弱項章節（quiz < 60）| 20 min |
| `/streak` | 看連續簽到 + 激勵 | 15 min |
| `/recommend` | AI 推薦下一課（基於弱項）| 40 min |
| `/quiz` | 隨機抽 1 題複習（從歷史錯題）| 1 hr |
| `/lesson <關鍵字>` | 找特定 lesson 直連網站 | 20 min |
| `/explain <概念>` | AI 一句話解釋（白話）| 20 min |
| `/quote` | 每日勵志程式名言 | 15 min |
| `/recap` | AI 用足跡寫一段話「你最近學了 X / Y / Z」 | 40 min |
| `/review` | 從 7+ 天前學的隨機挑 1 lesson、立刻出 quiz | 1.5 hr |
| `/forget` | 列「最該複習」（艾賓浩斯曲線）| 1 hr |
| 章節完成自動恭喜 | 完成一章 trigger push | 30 min |
| 連勝里程碑慶祝 | 7/30/100 天 push | 30 min |
| 連勝快斷時提醒 | 晚 22:00 沒簽到 push | 1 hr |
| 簽到隨機抽 Z-coin | 賭場心理 | 1 hr |
