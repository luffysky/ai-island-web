...
| LT-14 | **GDPR 匯出 / 刪除流程**（合規剛需、不可省）| 3+ 天 | ✅ code、SQL 待林董套用 |
...
| ID | 內容 | 估時 | 狀態 |
|---|---|---|---|
| P4-01 | 章節 / lesson 後台直接編輯（取代 JSON file-based）| ~2 週 | 🟡 編輯器 + 變更說明 ✅、JSON→DB cutover 留下階段 |
| P4-02 | 章節版本記錄 + diff | ~1 週 | ✅ code、SQL 待林董套用 |
...
| P4-10 | **錯誤日誌面板（剛需、雪鑰建議插隊）** | ~3 天 | ✅ code、SQL 待林董套用 |
| P4-11 | DB 健康 widget | ~1-2 天 | ❌ |
| P4-12 | 快取狀態 / clear cache 按鈕 | ~1 天 | ❌ |
| P4-13 | 環境變數面板（read-only mask） | ~1 天 | ❌ |
| P4-14 | **API rate limit per user（剛需、雪鑰建議插隊）** | ~3 天 | ✅ code、SQL 待林董套用 |
