...
- **目前進度：~1342 條 / 目標 5000。續寫從 `dictionary-seed-29.json` 接下去編號**（別重複既有 slug；批次主題見各 commit 訊息）。**「續辭典」= 從第 29 批繼續 author→import→commit→push。**（21 工程實務 27；22 後端/git 42；23 前端 39；24 AI/ML 31；25 DB/網路/資安 38；26 資料結構/演算法 48；27 雲端/DevOps/測試 49；28 敏捷/協作/文化/定律 46）
