...
- **目前進度：~1389 條 / 目標 5000。續寫從 `dictionary-seed-30.json` 接下去編號**（別重複既有 slug；批次主題見各 commit 訊息）。**「續辭典」= 從第 30 批繼續 author→import→commit→push。**（21 工程實務；22 後端/git；23 前端；24 AI/ML；25 DB/網路/資安；26 資料結構/演算法；27 雲端/DevOps/測試；28 敏捷/協作/文化/定律；29 函數式/JS內部/TS型別 47）
