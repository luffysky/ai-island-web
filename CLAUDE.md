...
7. **全部沒問題 → 先更新工作日誌（`docs/worklog/daily_works_*.md`）→ 再 commit / push。** 完成的 todo 用刪除線標記、不要刪。（待辦統一寫 `docs/todo/todo_list_0714.md`＝現行主檔；`todo_list_0713.md` 留作歷史）
...
- **目前進度：~1645 條 / 目標 5000。續寫從 `dictionary-seed-36.json` 接下去編號**（別重複既有 slug；批次主題見各 commit 訊息）。**「續辭典」= 從第 31 批繼續 author→import→commit→push。**（0713 一口氣加 21–30 批，主題：工程實務/後端·git/前端/AI·ML/DB·網路·資安/資料結構·演算法/雲端·DevOps·測試/敏捷·文化·定律/函數式·JS·TS型別/API·資料格式·編碼·時間）
