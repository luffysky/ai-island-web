...
- **目前進度：~1161 條 / 目標 5000。續寫從 `dictionary-seed-25.json` 接下去編號**（別重複既有 slug；批次主題見各 commit 訊息）。**「續辭典」= 從第 25 批繼續 author→import→commit→push。**（21 工程實務 27；22 後端/DevOps/git 42；23 前端/React/CSS/Web API 39；24 AI/ML 31）
