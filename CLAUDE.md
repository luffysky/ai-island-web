...
- **目前進度：~1512 條 / 目標 5000。續寫從 `dictionary-seed-33.json` 接下去編號**（別重複既有 slug；批次主題見各 commit 訊息）。**「續辭典」= 從第 31 批繼續 author→import→commit→push。**（0713 一口氣加 21–30 批，主題：工程實務/後端·git/前端/AI·ML/DB·網路·資安/資料結構·演算法/雲端·DevOps·測試/敏捷·文化·定律/函數式·JS·TS型別/API·資料格式·編碼·時間）
