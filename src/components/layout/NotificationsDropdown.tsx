...
export function NotificationsDropdown() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 max-w-[calc(100vw-2rem)] bg-bg-card border border-border rounded-xl shadow-2xl z-50">
                  ...
                  <li key={n.id} className={`p-3 hover:bg-bg-elevated text-xs ${!n.read_at ? "bg-accent/5" : ""}`}>
                    {n.link ? (
                      <Link href={n.link as any} onClick={() => setOpen(false)} className="flex items-start gap-2">
                        <span className="text-base flex-shrink-0">{KIND_EMOJI[n.kind] ?? "🔔"}</span>
