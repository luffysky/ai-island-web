...
export function CookieBanner() {
  ...
  return (
    <div
      ...
      className="fixed bottom-0 left-0 right-0 z-50 p-4 md:p-6 bg-[var(--color-bg-card)] border-t-2 border-[var(--color-accent)] shadow-2xl"
    >
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-4">
          <div className="flex-1 text-sm leading-relaxed">
            ...
            <p className="text-[var(--color-fg-muted)]">
              ...
              詳見 <Link href="/cookies" className="text-[var(--color-accent)] underline">Cookie 政策</Link>
              · <Link href="/privacy" className="text-[var(--color-accent)] underline">隱私權政策</Link>
            </p>

            {showDetails && (
              <div className="mt-3 p-3 bg-[var(--color-bg-elevated)] rounded text-xs space-y-2">
                <div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full md:w-auto shrink-0">
            <button
              ...
              className="px-4 py-2 text-sm border border-[var(--color-border)] rounded-lg hover:bg-[var(--color-bg-elevated)] transition"
            >
              {showDetails ? "收起" : "詳情"}
            </button>
            <button
            ...
            <button
              ...
              className="px-4 py-2 text-sm bg-[var(--color-accent)] text-black rounded-lg font-bold hover:scale-105 transition"
            >
              全部同意
            </button>
          </div>
