      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {Object.values(CAREER_PATHS).map((path) => (
          <Link
            key={path.id}
            href={`/career/${path.id}`}
            className="group p-5 rounded-xl bg-[var(--color-bg-card)] border border-[var(--color-border)] hover:border-[var(--color-accent)] transition-all hover:scale-[1.02]"
          >
            <div className="flex items-start justify-between mb-3">
              <div className="text-3xl">{path.emoji}</div>
              <div className="text-xs px-2 py-1 rounded bg-[var(--color-bg)] text-[var(--color-fg-muted)]">
                {path.chapters.length} 章
              </div>
            </div>
            <h3 className="font-bold text-lg mb-1">{path.name}</h3>
            <div className="text-sm text-[var(--color-accent)] mb-2">→ {path.title}</div>
            <p className="text-xs text-[var(--color-fg-muted)] leading-relaxed">{path.description}</p>
          </Link>
        ))}
      </div>
    </section>
