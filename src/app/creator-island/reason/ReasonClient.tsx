...
export function ReasonClient({ workspaceId, fragments, initialSelected = [] }: { workspaceId: string; fragments: Frag[]; initialSelected?: string[] }) {
  ...
  async function run() {
    if (selArr.length < 1) { setErr(t("reasonErrorPickAtLeastOne")); return; }
  ...
  async function feedback(idx: number, candidateId: string | undefined, verdict: "accepted" | "rejected") {
    ...
    try {
      ...
      if (!dbCand) throw new Error(t("reasonErrorCandidateNotFound"));
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 space-y-5">
      <header className="flex items-center justify-between gap-2 flex-wrap">
        <h1 className="text-2xl font-bold inline-flex items-center gap-2"><Brain size={22} className="text-accent" /> {t("reasonTitle")} <span className="text-xs font-normal text-fg-muted">{t("reasonSubtitle")}</span></h1>
        <Link href="/creator-island" className="text-sm text-accent hover:underline inline-flex items-center gap-1.5"><ArrowLeft size={14} /> {t("reasonBackToIsland")}</Link>
      </header>

      {/* 選碎片 */}
      <section className="space-y-2">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <div className="text-sm font-bold">{t("reasonPickHint")}</div>
          <div className="flex items-center gap-2">
            {fragments.length > 0 && (
              <button onClick={shuffle} title={t("reasonShuffleTitle")} className="text-xs inline-flex items-center gap-1 px-2.5 py-1 rounded-full border border-violet-400/40 bg-violet-500/10 text-violet-600 dark:text-violet-300 hover:bg-violet-500/20 transition"><Dices size={13} /> {t("reasonShuffle")}</button>
            )}
            <span className="text-xs text-fg-muted">{t("reasonFragCount", { n: fragments.length })}{selArr.length > 0 ? t("reasonSelectedCount", { m: selArr.length }) : ""}</span>
          </div>
        </div>
        {fragments.length === 0 ? (
          <p className="text-sm text-fg-muted bg-bg-card border border-border rounded-xl p-3">{t("reasonNoFragmentsPre")}<Link href="/creator-island" className="text-accent">{t("reasonNoFragmentsLink")}</Link>{t("reasonNoFragmentsPost")}</p>
        ) : (
          <>
            <div className="relative">
              <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-fg-muted" />
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t("reasonSearchPlaceholder")}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-[46vh] overflow-y-auto pr-1">
              ...
              {shownFrags.length === 0 && <p className="text-sm text-fg-muted col-span-full px-1 py-2">{t("reasonNoMatch", { q })}</p>}
            </div>
          </>
        )}
      </section>
      ...
      <section className="space-y-2">
        ...
        <input value={intent} onChange={(e) => setIntent(e.target.value)} placeholder={t("reasonIntentPlaceholder")}
      ...
      <button onClick={run} disabled={busy || selArr.length < 1}
        ...
        {busy ? <><Loader2 size={18} className="animate-spin" /> {t("reasonRunning")}</> : <><Sparkles size={18} /> {t("reasonStart", { n: selArr.length })}</>}
      </button>
      ...
      {result && (
        <section className="space-y-3">
          <div className="bg-bg-card border border-border rounded-2xl p-4">
            <div className="text-xs text-fg-muted mb-1">{t("reasonObservationLabel")}{MODES.find((m) => m.id === result.mode)?.label ?? result.mode}</div>
          ...
          {usedMem.length > 0 && (
            <div className="text-xs text-fg-muted bg-bg-card border border-border rounded-xl px-3 py-2">
              🧠 {t("reasonUsedMemory")}{usedMem.map((m) => m.text).join("；")}
            </div>
          )}

          <div className="text-sm font-bold">{t("reasonDirections", { n: result.candidates.length })}</div>
          {result.candidates.map((c, i) => (
            <div key={i} className="bg-bg-card border border-border rounded-2xl p-4 space-y-2">
              <div className="flex items-center justify-between gap-2 flex-wrap">
                ...
                <div className="text-[11px] text-fg-muted">{t("reasonConfidenceWeight", { conf: Math.round(c.confidence * 100), weight: c.weight })}</div>
              </div>
              <p className="text-sm whitespace-pre-wrap">{c.body}</p>
              {c.rationale && <p className="text-xs text-fg-muted">{t("reasonWhy")}{c.rationale}</p>}
              {c.missing?.length > 0 && <p className="text-xs text-amber-600 dark:text-amber-400">{t("reasonMissing")}{c.missing.join("、")}</p>}
              <div className="flex items-center gap-2 pt-1 flex-wrap">
                ...
                <button onClick={() => weave(i)} disabled={weaving !== null}
                  ...
                  {weaving === i ? <Loader2 size={13} className="animate-spin" /> : <PenLine size={13} />} {t("reasonWeave")}
                </button>
                {verdicts[i] ? (
                  <span className={`text-xs font-bold ${verdicts[i] === "accepted" ? "text-emerald-500" : "text-fg-muted"}`}>{verdicts[i] === "accepted" ? t("reasonAccepted") : t("reasonRejected")}</span>
                ) : (<>
                  <button onClick={() => feedback(i, undefined, "accepted")} className="text-xs px-3 py-1.5 rounded-full bg-emerald-500/15 text-emerald-600 dark:text-emerald-300 border border-emerald-500/30 inline-flex items-center gap-1"><Check size={13} /> {t("reasonAccept")}</button>
                  <button onClick={() => feedback(i, undefined, "rejected")} className="text-xs px-3 py-1.5 rounded-full bg-bg-elevated text-fg-muted border border-border inline-flex items-center gap-1"><X size={13} /> {t("reasonReject")}</button>
                </>)}
              </div>
            </div>
          ))}
          ...
          {trace.length > 0 && (
            <div className="bg-bg-card border border-border rounded-2xl p-3">
              <button onClick={() => setShowTrace((s) => !s)} className="text-sm font-bold inline-flex items-center gap-1.5 text-fg-muted">
                {showTrace ? <ChevronDown size={15} /> : <ChevronRight size={15} />} {t("reasonTrace", { n: trace.length })}
              </button>
              {showTrace && (
