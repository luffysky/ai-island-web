  ...
  return (
    <div className="grid sm:grid-cols-[220px_1fr] gap-3 min-h-[60vh]">
      ...
      <div className={`space-y-1 ${active ? "hidden sm:block" : ""}`}>
        {threads.length === 0 && <div className="text-xs text-fg-muted p-3">{tr("msgEmptyThreads")}</div>}
        {threads.map((t) => (
      ...
      <div className="flex flex-col bg-bg-card border border-border rounded-2xl overflow-hidden">
        {!active && <div className="grid place-items-center flex-1 text-sm text-fg-muted">{tr("msgSelectThread")}</div>}
        {active && (
          <>
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-2 max-h-[55vh]">
              ...
              {msgs.map((m) => (
                <div key={m.id} className={`max-w-[75%] rounded-2xl px-3 py-2 text-sm ${m.sender_id === meId ? "ml-auto bg-accent text-black" : "bg-bg-elevated"}`}>
                  ...
                  {m.media_url && m.media_type === "file" && <a href={m.media_url} target="_blank" className="underline text-xs block mt-1">{tr("msgAttachment")}</a>}
                </div>
              ))}
            </div>
