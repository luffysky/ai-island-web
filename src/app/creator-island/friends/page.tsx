...
import { listFriends, listPending } from "@/lib/creator-engine/friends";
...
export default async function FriendsPage() {
  ...
  const [friends, pending] = await Promise.all([listFriends(user.id), listPending(user.id)]);
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-8 space-y-5">
      ...
      <FriendsClient initialFriends={friends as any} initialPending={pending as any} />
    </div>
  );
}
