...
export default function PrivacyPage() {
  return (
    <div className="max-w-3xl mx-auto px-6 py-12 prose-custom">
      ...
      <h2>四、平台運營透明度（admin 可見性）</h2>
      <p>
        為了維護服務品質與防止濫用、本平台 admin（管理員）會收到下列即時通知：
      </p>
      <ul>
        <li><strong>使用者活動事件</strong>：登入、註冊、完課、升等、論壇回覆、訂單成立、AI 對話異常等</li>
        <li><strong>訪客活動</strong>：未登入訪客的訪問來源 IP、大致地理位置、裝置類型、停留時長（用於分析流量、偵測爬蟲 / 攻擊）</li>
        <li><strong>系統錯誤</strong>：自動偵測異常並通知 admin 排查</li>
      </ul>
      <p>
        這些通知<strong>不包含您的密碼、AI 對話內容、付款資訊</strong>等敏感資料、僅用於營運監控。
        若您不希望您的學習活動被通知 admin、可寄信至{" "}
        <code>luffysky00@gmail.com</code> 申請、本平台會在 30 日內處理。
      </p>
