...
export async function POST(req: NextRequest) {
  ...
  const { data: note } = await admin.from("notes").select("id, title, user_id").eq("id", invite.note_id).maybeSingle();
  ...
  return NextResponse.json({ ok: true, noteId: note.id, title: note.title, alreadyOwner: note.user_id === user.id });
}
