...
const DESKTOP_DOWNLOAD_URL = "https://github.com/luffysky/ai-island-bridge/releases/latest/download/ai-island-bridge-win.zip";
...
export function AgentClient() {
  ...
  return (
    <main className="max-w-6xl mx-auto px-4 py-6 sm:py-8">
      ...
      {/* 2.7.13 教學閉環：用 Agent → 學會建 Agent（畢業路徑，獨立可關） */}
      <FeatureGuide
        id="agent-build-loop"
        title="🎓 從「用」到「建」：打造你自己的 AI 員工"
        intro="你不只是使用者、也能當老闆——用著用著就學會怎麼把好用的做法變成你專屬的 AI 員工。跟著這 5 步走一輪，你就懂了。"
        steps={[
          { title: "① 先用範本跑一次", desc: "從範本庫挑一個任務送出，觀察它怎麼「拆步驟→挑工具→得到結果」。這就是一個 agent 的思考過程。" },
          { title: "② 把好用的做法存成技能", desc: "任務完成後，若做得不錯，按結果上的「💡 存成技能」——系統已自動幫你把這次做法整理成可重複用的員工草稿，一鍵採用。" },
          { title: "③ 到辦公室讓它自動上班", desc: "在「🏢 辦公室」給這位員工排程；想更聰明，開「🧠 讓員工自己決定要做什麼」，它會依職責自己找事做。" },
          { title: "④ 調成你的專屬員工", desc: "編輯它的職能（怎麼做事）、能用的工具、每日任務上限——像帶新人一樣把它調到位。" },
          { title: "⑤ 想更強？給它更多工具", desc: "貼一份 OpenAPI 網址或接 MCP server（右側工具區），它就能呼叫外部 API，能力再升級。" },
        ]}
        tip="閉環＝用它 → 看懂它怎麼做 → 存成技能 → 排程/自主 → 調教成專屬員工。你會從『下指令的人』變成『建 AI 團隊的人』。"
      />
