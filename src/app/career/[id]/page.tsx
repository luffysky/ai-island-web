  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      <div className="text-center mb-8">
        ...
        <div className="text-accent mb-3">→ {path.title}</div>
      ...
      <div className="space-y-2">
        {chapters.map((ch, i) => (
          <Link key={ch.id} href={`/chapters/${ch.id}`} className="flex items-center gap-4 p-4 rounded-lg bg-bg-card border border-border hover:border-accent">
            <div className="w-8 h-8 rounded-full bg-accent/20 text-accent flex items-center justify-center font-bold text-sm">{i + 1}</div>
