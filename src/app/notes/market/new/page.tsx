...
export default function NewNoteProductPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      setNotes((data ?? []).map((n: any) => ({ id: n.id, title: n.title?.trim() || "（無標題筆記）" })));
      setLoading(false);
    });
  }, [router]);
  ...
  async function publish() {
    if (!title.trim() || sel.size === 0) { setErr("缺標題 / 至少選一則筆記"); return; }
    ...
    try {
      ...
      if (!res.ok) throw new Error(j.message || j.error || "上架失敗");
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      ...
      <h1 className="text-2xl font-bold inline-flex items-center gap-2 mb-1"><Store size={22} className="text-accent" /> 上架我的筆記</h1>
      <p className="text-sm text-fg-muted mb-5">把一組整理好的筆記打包成商品。買家用 Z 幣購買、你 0 抽成收全額。</p>

      <div className="space-y-4">
        <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="商品標題（例：Python 新手完整筆記）" className="w-full bg-bg-card border border-border rounded-lg p-2.5 text-lg font-bold outline-none focus:border-accent" />
        <textarea value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="描述：這組筆記涵蓋什麼、適合誰" rows={2} className="w-full bg-bg-card border border-border rounded-lg p-2.5 text-sm outline-none focus:border-accent" />
        <label className="flex items-center gap-2 text-sm">價格
          <input type="number" min={0} max={100000} value={price} onChange={(e) => setPrice(Math.max(0, Math.min(100000, Number(e.target.value) || 0)))} className="w-24 bg-bg-card border border-border rounded-lg px-2 py-1.5 text-sm" /> Z 幣（0＝免費）
        </label>

        <div>
          <div className="flex items-center justify-between mb-2">
            <div className="text-sm font-bold">選要打包的筆記{sel.size > 0 ? `（已選 ${sel.size}）` : ""}</div>
            <div className="relative">
              <Search size={14} className="absolute left-2 top-1/2 -translate-y-1/2 text-fg-muted" />
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="搜尋" className="pl-7 pr-2 py-1 text-sm bg-bg-card border border-border rounded-lg outline-none focus:border-accent w-40" />
            </div>
          </div>
          {loading ? <div className="text-sm text-fg-muted py-6 text-center">載入你的筆記…</div> : (
            <div className="grid sm:grid-cols-2 gap-2 max-h-[46vh] overflow-y-auto pr-1">
              ...
              {shown.length === 0 && <div className="col-span-full text-sm text-fg-muted py-4 text-center">沒有符合的筆記。</div>}
            </div>
          )}
        </div>
        ...
        <button onClick={publish} disabled={busy || !title.trim() || sel.size === 0} className="w-full py-3 rounded-full bg-accent text-black font-bold disabled:opacity-40 inline-flex items-center justify-center gap-2">
          {busy ? <><Loader2 size={18} className="animate-spin" /> 上架中…</> : <>上架（{sel.size} 篇 · {price === 0 ? "免費" : `${price} Z`}）</>}
        </button>
      </div>
    </div>
  );
