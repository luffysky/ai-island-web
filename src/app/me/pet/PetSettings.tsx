...
export function PetSettings({ initial }: { initial: any }) {
  ...
  return (
    <div className="space-y-4">
      {/* 預覽卡 */}
      <div className="bg-gradient-to-br from-[var(--color-accent)]/15 via-[var(--color-accent-2)]/10 to-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl p-6 text-center">
        <div className="text-6xl mb-2">{species.emoji}</div>
        <div className="text-lg font-bold">{pet.name}</div>
        <div className="text-xs text-[var(--color-fg-muted)] mt-1">{species.name}</div>
        <div className="text-xs text-[var(--color-fg-muted)] mt-1">{species.intro}</div>
      </div>

      <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-2xl p-4 space-y-3">
        <div>
          <label className="text-xs text-[var(--color-fg-muted)] block mb-1">寵物名稱</label>
          <input
            type="text"
            maxLength={30}
            value={pet.name}
        ...
        <div>
          <label className="text-xs text-[var(--color-fg-muted)] block mb-2">物種</label>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-[var(--color-border)]">
          ...
          <button
            onClick={save}
            disabled={saving}
            className="px-5 py-1.5 text-sm bg-[var(--color-accent)] text-black font-bold rounded-lg disabled:opacity-50"
          >
            {saving ? "儲存中..." : "💾 儲存"}
          </button>
        </div>
      ...
      <div className="text-xs text-[var(--color-fg-muted)] bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-3">
