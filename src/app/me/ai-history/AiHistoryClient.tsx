...
export function AiHistoryClient({ initial }: { initial: Conv[] }) {
  ...
  const handleDelete = async (conv: Conv, e: React.MouseEvent) => {
    ...
    const ok = await confirm({
      title: t("aiHistoryDeleteTitle"),
      description: t("aiHistoryDeleteDesc", { title: conv.title || t("aiHistoryNoTitle") }),
    ...
    try {
      ...
      toast.success(t("aiHistoryDeleted"));
    } catch (err: any) {
      setConversations(snapshot);  // rollback
      toast.error(t("aiHistoryDeleteFailed"));
    } finally {
      setDeletingIds((s) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/me/ai-history/${selected.id}/messages`)
      ...
      .catch(() => toast.error(t("aiHistoryLoadMsgFailed")))
        ...
        <div className="p-2 border-b border-border">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t("aiHistorySearchConv")}
      ...
      <div className="rounded-xl bg-bg-card border border-border min-h-[400px]">
        {!selected ? (
          <div className="text-center py-24 text-fg-muted">
            ...
            <p className="text-sm">{t("aiHistorySelectPrompt")}</p>
          </div>
        ) : loadingMsgs ? (
          <div className="text-center py-24 text-fg-muted">
            <Loader2 size={16} className="animate-spin inline mr-1" /> {t("aiHistoryLoading")}
          </div>
        ) : (
          <>
            {messages.length > 3 && (
              <ChatToolbar
                ...
                exportText={messages.map((m) => `[${m.role === "user" ? t("you") : "AI"}] ${formatChatTime(m.created_at)}\n${m.content}`).join("\n\n")}
                exportFileName={`${(selected.title || "ai-chat").replace(/[^\w-]+/g, "_")}-${new Date().toISOString().slice(0, 10)}.txt`}
                placeholder={t("aiHistorySearchInConv")}
              />
            )}
            <div className="p-4 space-y-3 max-h-[600px] overflow-y-auto">
                ...
                .map((m, i) => (
                  ...
                  >
                    <div className="text-[10px] text-fg-muted mb-1 flex items-center gap-2">
                      <span className="font-bold">{m.role === "user" ? t("you") : "AI"}</span>
              ...
              {messages.length === 0 && (
                <div className="text-center text-fg-muted text-sm py-8">{t("aiHistoryNoMessages")}</div>
              )}
            </div>
          </>
        )}
  ...
  if (filtered.length === 0) {
    return (
      <div className="text-center py-12 text-fg-muted text-sm">
        {filter ? t("aiHistoryNoMatch") : t("aiHistoryEmpty")}
      </div>
    );
  }
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <button
                    ...
                    aria-label={t("aiHistoryDeleteConv")}
                    title={t("aiHistoryDeleteConv")}
