...
import { Loader2, CalendarHeart, RefreshCw, Link2, Dices } from "lucide-react";
  ...
  return (
    <div className="bg-gradient-to-br from-violet-500/15 via-pink-500/10 to-amber-500/15 border border-violet-500/30 rounded-2xl p-4">
      ...
      {loading ? (
        ...
        <div className="text-sm text-red-700 dark:text-red-300 py-2 flex items-center justify-between gap-2">
          <span>⚠️ {err}</span>
        ...
        <div className="text-sm text-fg-muted py-2 flex items-center justify-between gap-2">
          ...
          {fragmentCount >= 2 && (
            <button onClick={generate} className="inline-flex items-center gap-1 text-xs px-3 py-1.5 rounded-full bg-violet-500/30 text-violet-800 dark:text-violet-200 hover:bg-violet-500/50">
              <Dices className="w-3.5 h-3.5" /> 挑一個
            </button>
          )}
        </div>
      )}
