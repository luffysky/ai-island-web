...
export default async function MarketingHubPage() {
  ...
  const modules = [
    { href: "/admin/marketing/copy", emoji: "📝", title: "AI 文案產生器", desc: "同一主題 → FB / IG / X / Threads / LINE / Email / Blog 7 平台不同 copy" },
    { href: "/admin/marketing/schedule", emoji: "📅", title: "內容日曆 / 排程", desc: `${draftDraft} 草稿、${draftSched} 排程中、${draftPub} 已發佈` },
    { href: "/admin/marketing/publish", emoji: "📤", title: "多平台一鍵發佈", desc: "FB / IG / X / Threads / LINE OA / Blog / Email (OAuth 後啟用)" },
    { href: "/admin/marketing/ads", emoji: "🎯", title: "廣告 Copy 產生", desc: `Meta / Google / TikTok 廣告 copy、${adRunning} 個 running` },
    { href: "/admin/marketing/utm", emoji: "🔗", title: "UTM Builder", desc: `${utmRows.length} 條短連結、累積 ${utmClicks.toLocaleString()} 次點擊、轉換 ${utmConv} 筆、NT$ ${utmRev.toLocaleString()}` },
    { href: "/admin/marketing/brand", emoji: "🎨", title: "品牌風格庫", desc: "Brand voice / Tagline / Hashtag pool / 文案守則" },
    { href: "/admin/marketing/affiliates", emoji: "🤝", title: "推薦碼 / Affiliate", desc: `${affRows.length} 個有效推薦碼、佣金累積 NT$ ${affRev.toLocaleString()}` },
    { href: "/admin/marketing/competitor", emoji: "🔍", title: "競品 / 關鍵字", desc: `${compCount} 個競品快照、AI 抓取 + 摘要` },
  ];
  ...
  return (
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-pink-500/10 via-purple-500/10 to-blue-500/10 border border-purple-500/30 rounded-2xl p-5">
        <h1 className="text-2xl font-extrabold mb-1 flex items-center gap-2">🚀 行銷主控台</h1>
