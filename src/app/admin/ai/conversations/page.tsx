...
export default async function ConversationsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-sm text-[var(--color-fg-muted)]">最近 100 個對話、用來 audit user 跟 AI 怎麼互動</p>
      ...
      <div className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {convs?.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[var(--color-fg-muted)]">沒有對話</td></tr>
            ) : (
              convs?.map((c: any) => (
                <tr key={c.id} className="border-t border-[var(--color-border)] hover:bg-[var(--color-bg-elevated)]">
                  <td className="px-4 py-3 max-w-xs truncate">{c.title || "(無標題)"}</td>
                  <td className="px-4 py-3">
                    <Link href={`/admin/users?q=${c.profiles?.username}`} className="hover:text-[var(--color-accent)]">
                      {c.profiles?.display_name || c.profiles?.username || "—"}
                    </Link>
                  </td>
                  <td className="px-4 py-3 text-xs">{c.tone}</td>
                  <td className="px-4 py-3 text-xs">{c.use_byok ? "✓" : "—"}</td>
                  <td className="px-4 py-3 text-xs text-[var(--color-fg-muted)]">{new Date(c.updated_at).toLocaleString('zh-TW')}</td>
                </tr>
              ))
            )}
          </tbody>
