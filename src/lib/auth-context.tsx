...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // 不做 60s 強制 getUser() 踢登出 — Supabase JS SDK 自己會 refresh token
    // 之前那段在網路 flakey / Supabase 暫時 5xx 時會誤判把 user 踢出（林董抱怨「太快登出」）
    // 只有 onAuthStateChange 的 SIGNED_OUT event（user 主動登出 / token 真的 revoke）才會清狀態
