...
export interface LessonDemo {
  ...
  type: 'css-layout' | 'rwd-ruler' | 'box-model' | 'flex-playground' | 'grid-playground' | 'js-behavior' | 'prompt-lab' | 'prompt-builder' | 'funnel-sim' | 'decision-quiz' | 'scenario-judge' | 'priority-matrix' | 'color-contrast' | 'agent-loop' | 'workflow-flow' | 'json-tree' | 'regex-tester' | 'writing-studio';
...
export interface Lesson {
  ...
  // 互動體驗：引導式無碼教具（點/拉即體驗，如版面圖鑑、RWD 尺）
  demos?: LessonDemo[];
  // 即時測驗
  miniQuiz?: MiniQuiz;
  // 檔案範例
