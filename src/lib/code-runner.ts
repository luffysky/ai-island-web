/**
 * 多語言程式碼沙盒執行器（抽成 lib、給 /api/playground/run 與 agent `code.run` 工具共用）。
 * 三個後端、依序嘗試、前一個失敗自動退下一個：
 *  1. 自架 Piston（設 PISTON_BASE_URL）— 最快、可裝 runtime。
 *  2. Judge0（設 JUDGE0_KEY）— UTF-8 乾淨、品質好。
 *  3. Wandbox（免費、免 key）— 永遠墊底的保底後端。
 * 都是「外部隔離沙盒」執行、不碰本站資料/檔案；有時間與輸出限制。
 */
...
const FILE_EXTS: Record<string, string> = {
  python: "py", javascript: "js", go: "go", rust: "rs", java: "Main.java",
...
const JUDGE0_LANG: Record<string, number> = {
  bash: 46, python: 71, javascript: 63, "c++": 54, c: 50, go: 60, rust: 73, java: 62, csharp: 51,
  ruby: 72, php: 68, typescript: 74, lua: 64, sqlite3: 82, kotlin: 78, swift: 83, scala: 81, r: 80,
};

/** 可執行的語言別名清單（給 UI / 工具描述用）。 */
export const SANDBOX_LANGUAGES = Object.keys(VERSIONS);
...
// C# (mono) 預設把非 ASCII（中文）輸出成「?」——改寫 Console.Out 成 UTF-8 StreamWriter。
...
export type CodeRunResult =
  | { ok: true; stdout: string; stderr: string; exitCode: number | null; compile: { stdout: string; stderr: string; code: number } | null; language: string; filename: string; via: string }
  | { ok: false; error: string; status: number };

/**
 * 在外部隔離沙盒執行程式碼。回 { ok, stdout, stderr, exitCode, compile, via }。
 * language 接語言別名（見 SANDBOX_LANGUAGES）；code 上限 50KB。
 */
export async function runCode(input: { language: string; code: string; stdin?: string; filename?: string }): Promise<CodeRunResult> {
  const { language, code } = input;
  if (!language || !code) return { ok: false, error: "missing language or code", status: 400 };

  const langConfig = VERSIONS[String(language).toLowerCase()];
  if (!langConfig) return { ok: false, error: `不支援 ${language}`, status: 400 };
  if (code.length > 50000) return { ok: false, error: "Code 太長（max 50KB）", status: 400 };

  const ext = FILE_EXTS[langConfig.piston] ?? "txt";
  const finalCode = langConfig.piston === "csharp" ? fixCsharpUtf8(code) : code;
  const fname = resolveFilename(langConfig.piston, ext, finalCode, typeof input.filename === "string" ? input.filename : undefined);
  const st = input.stdin ?? "";
  ...
  for (const run of chain) {
    try {
      const out = await run();
      return { ok: true, stdout: out.stdout, stderr: out.stderr, exitCode: out.exitCode, compile: out.compile, language: langConfig.piston, filename: fname, via: out.via };
    } catch (e: any) {
      errors.push(e?.message ?? "failed");
    }
  }
  return { ok: false, error: `這個語言目前無法執行：${errors.join(" / ")}`, status: 502 };
}
