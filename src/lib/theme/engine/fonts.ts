...
export function compileFontVars(assignment: FontAssignment): Record<string, string> {
  const vars: Record<string, string> = {
    '--font-heading': buildFontFamily(assignment.heading, assignment.latin),
    '--font-body': buildFontFamily(assignment.body, assignment.latin),
    '--font-ui': buildFontFamily(assignment.ui, assignment.latin),
  }

  // mono 不套拉丁字體 —— 等寬的意義就在於每個字元同寬，
  // 混進另一套字體會破壞對齊。
  if (assignment.mono) {
    vars['--font-mono'] = buildFontFamily(assignment.mono)
  }
